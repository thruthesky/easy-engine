...
import { Payload, PayloadNotification, SendMessageRequest, SendMessageToSubscription, SendMessageToUidsRequest } from "./messaging.interfaces";
 ...
 */
     ...
     * 
     * @param {string} req.concurrentConnections - The size of chunk. Default 500. 기본 500 개 단위로 chunk. sendEach() 가 최대 500개 까지 지원
     *
     * @returns {string | undefined} - 로그를 기록하고 그 결과를 리턴한다.
     */
    static async sendMessageToUids(req: SendMessageToUidsRequest)
        : Promise<string[]> {
        // prepare the parameters
        let { uids, concurrentConnections, title, body, image, data } = req;
        data = data ?? {};
        concurrentConnections = concurrentConnections ?? 500;
    ...
    static async sendMessage(params: SendMessageRequest): Promise<string[]> {
        ...
        // Image is optional
        const notification: PayloadNotification = { title: params.title, body: params.body };
        if (params.image) {
    ...
    static async sendMessageToTokens(messages: Array<Payload>): Promise<string[]> {
        ...
        const tokensWithError: string[] = messages.filter((message) => {
        ...
        if (Config.removeTokensWithErrors) {
            ...
            for (let i = 0; i < tokensWithError.length; i++) {
                promisesToRemove.push(getDatabase().ref(`${Config.fcmTokens}/${tokensWithError[i]}`).remove());
            }
     ...
     * @param chunkSize chunk size - default 128.
